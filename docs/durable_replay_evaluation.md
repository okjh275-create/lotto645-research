...
## Durable replay result artifact promotion action-plan source adapter

`DurableReplayResultArtifactPromotionActionPlanSourceAdapter` is a composition-only operations adapter. It accepts explicit `artifact_root` and `end_round` values, forwards both values unchanged to `DurableReplayResultArtifactPromotionEligibilitySourceAdapter.adapt`, forwards the resulting eligibility unchanged to `DurableReplayResultPromotionActionPlanService.plan`, and returns the resulting `DurableReplayResultPromotionActionPlan` unchanged.

The adapter does not construct lower-layer artifact or comparison objects, does not reconstruct action-plan fields, and does not own action-plan policy. It does not build `DurableReplayPromotionPublicationRequest`, handle `source_decision` or `registry_root`, perform publication lifecycle operations, mutate the production registry, or perform rollback. It also does not perform filesystem I/O, parse JSON, verify manifests, discover artifact directories, normalize or expand paths, infer `end_round`, or expose CLI/stdin/stdout behavior. Existing lower-layer and downstream owners remain authoritative and unchanged.
